...
{
    std::shared_ptr<Spremnik> pspremnik;
    void Test() const
    {
        if(!pspremnik) throw std::logic_error("Nespecificiran spremnik");
    }
public:
    PolimorfniSpremnik() : pspremnik(nullptr) {}
    //~PolimorfniSpremnik(){
    //delete pspremnik;
    //  }
    PolimorfniSpremnik(const Spremnik& s) : pspremnik(s.DajKopiju()) {}
    PolimorfniSpremnik(const PolimorfniSpremnik& p)
    {
        if(!p.pspremnik) pspremnik=nullptr;
        else pspremnik=p.pspremnik->DajKopiju();
    }
    PolimorfniSpremnik(PolimorfniSpremnik&& p)
    {
        pspremnik=p.pspremnik;
        p.pspremnik=nullptr;
    }
    PolimorfniSpremnik& operator =(PolimorfniSpremnik s)
    {
        //std::shared_ptr<Spremnik> p=nullptr; // deklaracija pokazivaca na spremnik
        // if(!s.pspremnik) p=s.pspremnik->DajKopiju(); //pravi se duboka kopija i preuzima pok na tu kopiju
        //    delete pspremnik;
        // pspremnik=p;
        std::swap(pspremnik,s.pspremnik);
        return *this;
    }
    /*   PolimorfniSpremnik& operator =(PolimorfniSpremnik&& s){
           std::swap(pspremnik,s.pspremnik);
    ...
    {
        Test();
        return pspremnik->DajUkupnuTezinu();
    }
    void Ispisi()
    {
        Test();
        return pspremnik->Ispisi();
    }
    double DajTezinu()
    {
        Test();
        return pspremnik->DajTezinu();
    }
...
int main ()
{
    ...
    PolimorfniSpremnik s1(Bure(7,"Voda", 154, 29));
    PolimorfniSpremnik s2=Vreca(3,"Pijesak",10);
    try {
        s1.Ispisi();
        s2.Ispisi();
        std::cout << s1.DajTezinu() << "        " << s2.DajTezinu() << std::endl;
        s1=s2;
        std::cout << s1.DajUkupnuTezinu() << "        " <<
                  s2.DajUkupnuTezinu();
