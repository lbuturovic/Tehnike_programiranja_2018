...
int main ()
{
	...
	/*std::shared_ptr<Pok> rec;*/
		int n;
		std::cin >> n;
		std::cout << "Unesite recenice: " << std::endl;
		std::cin.ignore(1000,'\n');
		try{
		std::shared_ptr<Pok> rec (new Pok [n], [] (Pok *rec){delete [] rec;});
		...
		rec[i]=nullptr;*/
			...
			i onda ona upisati u zagradu--------------------| (pada cetvrti test)*/
				rec.get()[i]= std::make_shared <std::string>(std::string());
				std::getline(std::cin, *rec.get()[i]);
				/*rec[i]= pomocna;*/
				/*std::cout << rec[i];*/
				brojac++;
