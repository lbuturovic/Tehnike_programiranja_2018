...
public:
    ...
    void RegistrirajNovogStudenta(int indeks, std::string godina, std::string ime, std::string adresa, std::string broj)
    {
        ...
        std::shared_ptr<Student> pok=std::make_shared<Student>(indeks, godina, ime, adresa, broj);
        studenti[indeks]=pok;
    }
    void RegistrirajNoviLaptop(int evidencija, std::string naz, std::string kar)
    {
        ...
        std::shared_ptr<Laptop> lt=std::make_shared<Laptop>(evidencija, naz, kar);
...
Administracija::Administracija(const Administracija &adm)
{
    for(auto it=adm.studenti.begin(); it!=adm.studenti.end(); it++) {
        std::shared_ptr<Student> pok=std::make_shared<Student>(*(it->second));
        studenti[it->first]=pok;
    }
    for(auto it=adm.laptopi.begin(); it!=adm.laptopi.end(); it++) {
        std::shared_ptr<Laptop> pok=std::make_shared<Laptop>(*(it->second));
...
void Administracija::ZaduziLaptop(int indeks, int evidencija)
{
    ...
    std::shared_ptr<Student> st=studenti[indeks];
    for(auto it=laptopi.begin();it!=laptopi.end();it++)
    if((it->second)->DajPokKodKogaJe()==st.get()) throw std::domain_error ("Student je vec zaduzio laptop");
